...
int display_env(char **args, char __attribute__((__unused__)) **front)
{
	...
	{
		write(STDOUT_FILENO, environ[index], get_string_length(environ[index]));
...
/**
 * display_setenv - Changes or adds an environmental variable to the PATH.
 * @args: An array of arguments passed to the shell.
 * @front: A double pointer to the beginning of args.
...
/**
 * display_unsetenv - Deletes an environmental variable from the PATH.
 * @args: An array of arguments passed to the shell.
 * @front: A double pointer to the beginning of args.
