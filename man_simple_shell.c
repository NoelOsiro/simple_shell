.TH MY_SHELL 1 "November 2018" "Holberton School" "0x15. C - Simple Shell"
.SH NAME
.B myshell - simple UNIX command interpreter
.SH SYNOPSIS
.B myshell \fR[\fIfilename\fR]
.SH DESCRIPTION
.B MyShell \fR is a simple UNIX command language interpreter that reads commands from either a file or standard input and executes them.

.B Invocation
.in +2n
\fBMyShell \fRcan be invoked both interactively and non-interactively.
If \fBMyShell \fRis invoked with standard input not connected to a terminal, it reads and executes received commands in order.

If \fBMyShell \fRis invoked with standard input connected to a terminal (determined by \fIisatty(3)\fR), an \fIinteractive\fR shell is opened.
When executing interactively, \fBMyShell \fRdisplays the prompt \fI$ \fR when it is ready to read a command.

Alternatively, if command line arguments are supplied upon invocation, \fBMyShell \fRtreats the first argument as a file from which to read commands.
The supplied file should contain one command per line.
.B MyShell \fRruns each of the commands contained in the file in order before exiting.
.in

.B Environment
.in +2n
Upon invocation, \fBMyShell \fRreceives and copies the environment of the parent process in which it was exeucted.
This \fBenvironment\fR is an array of \fIname-value\fR strings describing variables in the format \fINAME=VALUE\fR.
.in

.B Command Execution
.in +2n
After receiving a command, \fBMyShell \fRtokenizes it into words using \fB" "\fR as a delimiter.
The first word is considered the command and all remaining words are considered arguments to that command.
.B MyShell \fRthen proceeds with the following actions:

.RS

If the first character of the command is neither a slash (\fI/\fR) nor dot (\fI.\fR), the shell searches for it in the list of shell builtins.
If there exists a shell builtin by that name, the builtin is invoked.
.RE
.RS
2. If the first character of the command is none of a slash (\fI/\fR), dot (\fI.\fR), nor builtin, \fBMyShell \fRsearches each element of the
...
.B Exit Status
.in +2n
.B MyShell \fRreturns the exit status of the last command executed, unless a syntax error occurs, with zero indicating success and non-zero indicating failure.
...
.B Signals
.in +2n
While running in interactive mode, \fBMyShell \fRignores the keyboard input \fBCtrl+c\fR.
Alternatively, an input of end-of-file (\fBCtrl+d\fR) will exit the program.
.in

.B Variable Replacement
.in +2n
.B MyShell \fRinerprets the \fI$\fR character for variable replacement:
...
.B Comments
.in +2n
.B MyShell \fRignores all words and characters preceeded by a \fI#\fR character on a line.
.in
...
.B MyShell \fRemulates basic functionality of the \fBsh\fR shell.
