 ...
 * @my_shell_name: The my_shell_name of the new alias to be added.
...
alias_t *add_alias_end(alias_t **head, char *my_shell_name, char *value)
{
	...
	new_node->next = NULL;
	new_node->my_shell_name = malloc(sizeof(char) * (my_strlen(my_shell_name) + 1));
	if (!new_node->my_shell_name)
	{
		free(new_node);
		return (NULL);
	}
	new_node->value = value;
	my_strcpy(new_node->my_shell_name, my_shell_name);
